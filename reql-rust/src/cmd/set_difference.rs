...
#[derive(Debug, Clone)]
pub struct SetDifferenceBuilder<T>(pub(crate) Command, pub(crate) PhantomData<T>);

impl<T: Unpin + Serialize + DeserializeOwned> SetDifferenceBuilder<T> {
    pub(crate) fn new(values: &[impl Serialize]) -> Self {
        let arg = Command::from_json(values);
        let command = Command::new(TermType::SetDifference).with_arg(arg);
...
impl<T: Unpin + Serialize + DeserializeOwned> ReqlOpsSequence<T> for SetDifferenceBuilder<T> {}
...
impl<T> Into<Command> for SetDifferenceBuilder<T> {
