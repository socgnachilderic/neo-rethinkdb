    ...
    B: Unpin + Serialize + DeserializeOwned,
{
    pub(crate) fn new(base: A, func: Func) -> Self {
        ...
        let Func(func) = func;
        let command = Command::new(TermType::Fold).with_arg(arg).with_arg(func);
...
impl<A, B> ReqlOps for FoldBuilder<A, B> {
    fn get_parent(&self) -> Command {
        self.0.clone().into_arg::<()>().into_cmd()
    }
}

impl<A, B> Into<Command> for FoldBuilder<A, B> {
